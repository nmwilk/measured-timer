...
public class RotatableImageView extends ImageView
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        // draw the image that's rotated (background not drawn in onDraw).
        super.onDraw(canvas);
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        {
            if (mIgnoreMask.getPixel((int) x, (int) y) == Color.WHITE) return true;
        }
        ...
        switch (actionMasked)
        {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                if (!mUsedDownAction) return true;

                mStartAngle2 = snapAngle(mAngle2
                        + CoordTools.getAngleFromVelocity(event.getX(1) - mCentreX, event.getY(1) - mCentreY));
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                for (int i = 0; i < ptrCount; i++)
                {
                    ...
                    {
                        float newAngle2 = snapAngle(mStartAngle2
                                - CoordTools.getAngleFromVelocity(event.getX(i) - mCentreX, event.getY(i) - mCentreY));
                        diffAngle2 = newAngle2 - mAngle2;
                        mAngle2 = newAngle2;
                    }
                    else
                    {
                        float newAngle1 = snapAngle(mStartAngle1
                                - CoordTools.getAngleFromVelocity(x - mCentreX, y - mCentreY));
                ...
                if (mAngle < 0)
                    mAngle += 360;
                else if (mAngle > MAX_ANGLE) mAngle -= MAX_ANGLE;

                break;
        }

        if (mAnglePrev != mAngle) this.invalidate();
